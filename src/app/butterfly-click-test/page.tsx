...
export default function ButterflyClickTestPage() {
  ...
  const startTest = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (elapsed > 0) {
        const c = parseFloat((clicksRef.current / elapsed).toFixed(2));
        setCps(c);
        setBestCps(prev => Math.max(prev, c));
      }
      if (remaining <= 0) {
      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-8 pt-28 pb-20">
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-primary">Butterfly Click Test</span>
          </div>
          <span className="inline-flex items-center gap-2 glass-card rounded-full px-3 py-1.5 text-xs font-semibold text-rose-400 uppercase tracking-widest mb-4">Gaming</span>
          <h1 className="text-section-title font-bold text-foreground mb-3">
            Butterfly Click <span className="text-primary">Test</span>
          </h1>
          <p className="text-muted-foreground text-base leading-relaxed max-w-xl">
            Test butterfly clicking — alternating two fingers on the same mouse button for maximum CPS.
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { val: clicks, label: 'Clicks', color: 'text-primary' },
            { val: cps.toFixed(2), label: 'CPS', color: 'text-rose-400' },
            { val: bestCps.toFixed(2), label: 'Best CPS', color: 'text-amber-400' },
            { val: `${timeLeft.toFixed(1)}s`, label: 'Time', color: 'text-violet-400' },
          ].map(s => (
            <div key={s.label} className="glass-card rounded-2xl p-4 text-center">
              <div className={`font-mono text-xl font-bold ${s.color} mb-1`}>{s.val}</div>
        ...
        <div className="flex justify-center mb-8">
          <button
            ...
            className={`w-72 h-72 rounded-full border-2 transition-all duration-75 select-none flex flex-col items-center justify-center gap-2 ${
              phase === 'running' ? 'bg-rose-500/10 border-rose-400 active:scale-95 shadow-[0_0_40px_rgba(244,63,94,0.3)]' :
              phase === 'done'? 'bg-emerald-500/10 border-emerald-400 cursor-default' : 'glass-card border-border hover:border-rose-400/40 active:scale-95'
            }`}
          >
            <div className="cps-counter text-rose-400">{cps.toFixed(1)}</div>
